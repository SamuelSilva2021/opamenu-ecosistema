...
export function OrderTypeSelector({ value, onChange }: OrderTypeSelectorProps) {
    return (
        <div className="grid w-full grid-cols-3 gap-1 bg-muted p-1 rounded-lg">
             <Button
                variant="ghost"
                size="sm"
             ...
             <Button
                variant="ghost"
                size="sm"
                className={cn("gap-2", value === OrderType.Table && "bg-background shadow-sm")}
                onClick={() => onChange(OrderType.Table)}
             >
