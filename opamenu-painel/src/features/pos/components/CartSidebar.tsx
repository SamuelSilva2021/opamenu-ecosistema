...
export function CartSidebar() {
    ...
    const fetchNextOrderNumber = async () => {
        try {
            const response = await posService.getNextOrderNumber();
            if (response.succeeded) {
                setNextOrderNumber(response.data);
            }
        } catch (error) {
            console.error("Erro ao buscar próximo número de pedido", error);
        }
    };
            ...
            <div className="p-5 space-y-4 border-b bg-muted/5">
                <div className="flex items-center justify-between">
                    ...
                    <div className="text-xs font-medium px-2 py-1 bg-primary/10 text-primary rounded-full">
                        #{nextOrderNumber ? nextOrderNumber.toString().padStart(3, '0') : "..."}
                    </div>
                </div>
