...
import { AditionalGroupType } from "../types";
...
interface AditionalGroupFormProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (data: CreateAditionalGroupRequest | UpdateAditionalGroupRequest) => void;
  initialData?: AditionalGroup | null;
  isLoading?: boolean;
}

export function AditionalGroupForm({
  open,
  onOpenChange,
  onSubmit,
  initialData,
  isLoading,
}: AditionalGroupFormProps) {
  const form = useForm<z.infer<typeof formSchema>>({
    resolver: zodResolver(formSchema) as Resolver<z.infer<typeof formSchema>>,
    defaultValues: {
      name: "",
      description: "",
      type: AditionalGroupType.Multiple,
      minSelections: 0,
      maxSelections: 0,
      isRequired: false,
      isActive: true,
  ...
  const handleSubmit = (values: z.infer<typeof formSchema>) => {
    onSubmit({
      ...
      type: values.type as AditionalGroupType,
    });
  };
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value={String(AditionalGroupType.Single)}>
                          Única
                        </SelectItem>
                        <SelectItem value={String(AditionalGroupType.Multiple)}>
                          Múltipla
                        </SelectItem>
                      </SelectContent>
                    </Select>
