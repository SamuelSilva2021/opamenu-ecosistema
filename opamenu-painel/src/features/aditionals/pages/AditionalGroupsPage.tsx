...
import type { AditionalGroup, CreateAditionalGroupRequest, UpdateAditionalGroupRequest } from "../types";
import { AditionalGroupForm } from "../components/AditionalGroupForm";
import { AditionalManager } from "../components/AditionalManager";
...
export default function AditionalGroupsPage() {
  const { can } = usePermission();
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [editingGroup, setEditingGroup] = useState<AditionalGroup | null>(null);
  const [deleteId, setDeleteId] = useState<string | null>(null);
  const [manageAditionalsGroupId, setManageAditionalsGroupId] = useState<string | null>(null);
  ...
  const createMutation = useMutation({
    mutationFn: aditionalsService.createGroup,
    onSuccess: (response) => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: UpdateAditionalGroupRequest }) =>
      aditionalsService.updateGroup(id, data),
    onSuccess: (response) => {
      queryClient.invalidateQueries({ queryKey: ["aditional-groups"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: aditionalsService.deleteGroup,
    onSuccess: (response) => {
      queryClient.invalidateQueries({ queryKey: ["aditional-groups"] });
  ...
  const handleCreate = (data: CreateAditionalGroupRequest | UpdateAditionalGroupRequest) => {
    if (editingGroup) {
      updateMutation.mutate({ id: editingGroup.id, data: data as UpdateAditionalGroupRequest });
    } else {
      createMutation.mutate(data as CreateAditionalGroupRequest);
    }
  };

  const openEditForm = (group: AditionalGroup) => {
  ...
  const columns: ColumnDef<AditionalGroup>[] = [
    {
      accessorKey: "name",
      header: ({ column }) => {
        return (
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              {canEdit && (
                <>
                  <DropdownMenuItem onClick={() => setManageAditionalsGroupId(group.id)}>
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <AditionalManager
        groupId={manageAditionalsGroupId}
        open={!!manageAditionalsGroupId}
        onOpenChange={(open: boolean) => !open && setManageAditionalsGroupId(null)}
      />
