    ...
    ICurrentUserService currentUserService) : ITenantBusinnessService
{
    ...
    public async Task<ResponseDTO<TenantBusinessResponseDto>> UpdateTenantBusinessInfo(UpdateTenantBusinessRequestDto dto)
    {
        try
        {
            ...
            if (tenantId == null) return StaticResponseBuilder<TenantBusinessResponseDto>.BuildError("Tenant não identificado.");
            ...
            if (!string.IsNullOrEmpty(dto.PixKey))
            {
                var bankDetailsEntities = await bankDetailsRepository.GetByTenantIdAsync(tenant.Id);

                var bankDetails = bankDetailsEntities.FirstOrDefault(b => b.PixKey == dto.PixKey);

                if (bankDetails != null)
                {
                    foreach(var bank in bankDetailsEntities)
                    {
                        bank.IsPixKeySelected = bank.PixKey == dto.PixKey;
                        bank.UpdatedAt = DateTime.UtcNow;
                    }
                    await bankDetailsRepository.UpdateRangeAsync(bankDetailsEntities);
                }
                else
                {
                    var bankDetais = new BankDetailsEntity
                    {
                        TenantId = tenant.Id,
                        PixKey = dto.PixKey,
                        IsPixKeySelected = true,
                    };
                    await bankDetailsRepository.AddAsync(bankDetais);

                    foreach (var bank in bankDetailsEntities)
                    {
                        bank.IsPixKeySelected = bank.PixKey == bankDetais.PixKey;
                        bank.UpdatedAt = DateTime.UtcNow;
                    }
                    await bankDetailsRepository.UpdateRangeAsync(bankDetailsEntities);
                }
