    ...
    ) : ICouponService
{
    ...
    public async Task<ResponseDTO<IEnumerable<CouponDto>>> GetAllAsync()
    {
        try
        {
            ...
            if (tenantId == null)
                return StaticResponseBuilder<IEnumerable<CouponDto>>.BuildError("Tenant nÃ£o identificado.");
    ...
    public async Task<ResponseDTO<CouponDto?>> GetByIdAsync(int id)
    {
        try
        {
            var tenantId = _currentUserService.GetTenantGuid();
            if (tenantId == null)
                return StaticResponseBuilder<CouponDto?>.BuildError("Tenant nÃ£o identificado.");

            var coupon = await _couponRepository.GetByIdAsync(id, tenantId.Value);
            if (coupon == null)
                return StaticResponseBuilder<CouponDto?>.BuildError("Cupom nÃ£o encontrado.");
    ...
    public async Task<ResponseDTO<CouponDto>> CreateAsync(CreateCouponRequestDto dto)
    {
        try
        {
            ...
            if (!await _couponRepository.IsCodeUniqueAsync(dto.Code, tenantId.Value))
                return StaticResponseBuilder<CouponDto>.BuildError("JÃ¡ existe um cupom com este cÃ³digo.");
    ...
    public async Task<ResponseDTO<CouponDto>> UpdateAsync(int id, UpdateCouponRequestDto dto)
    {
        try
        {
            var tenantId = _currentUserService.GetTenantGuid();
    ...
    public async Task<ResponseDTO<bool>> DeleteAsync(int id)
    {
        try
        {
            var tenantId = _currentUserService.GetTenantGuid();
            if (tenantId == null)
                return StaticResponseBuilder<bool>.BuildError("Tenant nÃ£o identificado.");
    ...
    public async Task<ResponseDTO<IEnumerable<CouponDto>>> GetActiveCouponsForStorefrontAsync()
    {
        try
        {
            var tenantId = _currentUserService.GetTenantGuid();
            if (tenantId == null)
                return StaticResponseBuilder<IEnumerable<CouponDto>>.BuildError("Loja nÃ£o identificada.");
    ...
    public async Task<ResponseDTO<CouponDto?>> ValidateCouponAsync(string code, decimal orderValue)
    {
        try
        {
            var tenantId = _currentUserService.GetTenantGuid();
            if (tenantId == null)
                return StaticResponseBuilder<CouponDto?>.BuildError("Loja nÃ£o identificada.");
    ...
    public async Task<ResponseDTO<IEnumerable<CouponDto>>> GetActiveCouponsBySlugAsync(string slug)
    {
        try
        {
            ...
            if (tenantId == Guid.Empty)
                 return StaticResponseBuilder<IEnumerable<CouponDto>>.BuildError("Loja nÃ£o encontrada.");
    ...
    public async Task<ResponseDTO<CouponDto?>> ValidateCouponBySlugAsync(string slug, string code, decimal orderValue)
    {
        try
        {
            var tenantId = await _tenantRepository.GetTenantIdBySlugAsyn(slug);
            if (tenantId == Guid.Empty)
                 return StaticResponseBuilder<CouponDto?>.BuildError("Loja nÃ£o encontrada.");
    ...
    {
        if (coupon == null)
            return StaticResponseBuilder<CouponDto?>.BuildError("Cupom invÃ¡lido.");
        ...
        if (coupon.StartDate.HasValue && coupon.StartDate > now)
            return StaticResponseBuilder<CouponDto?>.BuildError("Cupom ainda nÃ£o estÃ¡ vÃ¡lido.");
        ...
        if (coupon.MinOrderValue.HasValue && orderValue < coupon.MinOrderValue)
            return StaticResponseBuilder<CouponDto?>.BuildError($"Valor mÃ­nimo do pedido para este cupom Ã© {coupon.MinOrderValue:C}.");
