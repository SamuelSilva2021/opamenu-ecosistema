...
export const TenantProductForm = ({ initialData, onSubmit, onCancel, isLoading }: TenantProductFormProps) => {
  ...
  const configurationSchemaValue = watch('configurationSchema') || '';
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700">Taxa de Instalação</label>
          <input
            type="number"
            step="0.01"
            {...register('setupFee')}
